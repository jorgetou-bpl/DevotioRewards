...
const COLORS = ['#8CA4FE', '#5B7CF7', '#0B0B16'];

// Lifetime visit-count buckets per customer (1 / 2-3 / 4+), same shape as
// AgeDistributionChart — a snapshot of customer_stats.total_visits, not
// filtered by the page's date range (recurrence describes the customer, not
// a period).
export const RecurrenceChart = ({ data, loading }) => {
  const hasData = data?.some((d) => d.count > 0);

  return (
    <div className="card-brutalist">
      <h3 className="text-lg font-semibold text-[#0B0B16] mb-4 flex items-center gap-2">
        <Repeat className="h-5 w-5 text-[#8CA4FE]" />
      ...
      {loading ? (
        ...
        <p className="text-zinc-500 text-sm text-center py-8">
          Aún no hay suficientes datos de visitas
        </p>
      )}
    </div>
  );
